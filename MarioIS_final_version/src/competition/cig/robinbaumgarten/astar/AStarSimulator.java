...
public class AStarSimulator 
{
    ...
    public ArrayList<SearchNode> posPool;		// the open-list of A*, contains all the unexplored search nodes
    /***subtle point***/ArrayList<int[]> visitedStates = new ArrayList<int[]>(); // the closed-list of A*
	...
	private class SearchNode
	{
		...
		/***subtle point***/public boolean isInVisitedList = false;
		...
		float maxForwardMovement = 10.09f;
		 	 	
		public SearchNode(boolean[] action, int repetitions, SearchNode parent)
		{
			/*System.out.println("INSIDE SearchNode");
			if (parent != null)
			{
				System.out.println("parent.sceneSnapshot.mario.x IS: " + parent.sceneSnapshot.mario.x);
			}
			System.out.println("levelScene.mario.x IS: " + levelScene.mario.x);*/
			
	    	this.parentPos = parent;
	    	if (parent != null)
	    	{
	    		this.remainingTimeEstimated = parent.estimateRemainingTimeChild(action, repetitions);	    		
	    		timeElapsed = parent.timeElapsed + (int)(maxForwardMovement/maxMarioSpeed);//+ repetitions;
	    	}
	    	else
	    	{
		...
		public float calcRemainingTime(float marioX, float marioXA)
		{
			...
			// trying with my own heuristic
			float largeDistance = 100000.0f;
			return (largeDistance - marioX)/(maxMarioSpeed);
		}
		...
		{
			/*System.out.println("#*******************************************#");
			System.out.println("RUNNING simulatePos");
			System.out.println("levelScene.mario.x BEFORE: " + levelScene.mario.x);
			System.out.println("parentPos.sceneSnapshot.mario.x BEFORE: " + parentPos.sceneSnapshot.mario.x);
			System.out.println("sceneSnapshot BEFORE: " + sceneSnapshot);*/
			
			//System.out.println("#*******************************************#");
	    	// set state to parents scene
			//System.out.println("levelScene BEFORE: " + levelScene);
			//System.out.println("parentPos.sceneSnapshot BEFORE: " + parentPos.sceneSnapshot);
			
			levelScene = parentPos.sceneSnapshot;
			//System.out.println("levelScene AFTER: " + levelScene);
			parentPos.sceneSnapshot = backupState();
			//System.out.println("parentPos.sceneSnapshot AFTER: " + parentPos.sceneSnapshot);
			
			int initialDamage = getMarioDamage();
			//System.out.println("repetitions IS::::::::::::::::::::::::::: " + repetitions);
	    	for (int i = 0; i < 2/*repetitions*/; i++)
	    	{
	    		...
	    		//System.out.println("SIM BEFORE: " + levelScene);
	    		//System.out.println("levelScene.mario.x BEFORE: " + levelScene.mario.x);
	    		// Run the simulator
	    		advanceStep(action);
	    		
	    		// after running simulator
	    		//System.out.println("SIM AFTER: " + levelScene);
	    	...
	    	remainingTime = calcRemainingTime(levelScene.mario.x, levelScene.mario.xa)
	    	 	+ (getMarioDamage() - initialDamage) * damageCost;//(1000000 - 100 * timeElapsed);
	    	if (isInVisitedList)
	    	...
	    	/*System.out.println("levelScene.mario.x AFTER: " + levelScene.mario.x);
			System.out.println("parentPos.sceneSnapshot.mario.x AFTER: " + parentPos.sceneSnapshot.mario.x);
			System.out.println("sceneSnapshot.mario.x AFTER: " + sceneSnapshot.mario.x);
			System.out.println("#*******************************************#");*/	
	    	return remainingTime;			
		}
    ...
    {
    	System.out.println("INSIDE A* SEARCH!");
    	System.out.println("bestPosition.sceneSnapshot.mario.x IS: " + bestPosition.sceneSnapshot.mario.x);
    	System.out.println("bestPosition.parentPos IS: " + bestPosition.parentPos);
    	//SearchNode current = bestPosition;
    	//System.out.println("current.sceneSnapshot.mario.x INITIAL: " + current.sceneSnapshot.mario.x);
    	boolean currentGood = false;		// is the current node good (= we're not getting hurt)
    	int ticks = 0;
    	int maxRight = 176;					// distance to plan to the right (= 16*11, from the center to the right end. Finding 16 by an accident!)
    	
    	// Search until we've reached the right side of the screen, or if the time is up.
    	System.out.println("posPool.size() IS: " + posPool.size());
    	float temp1 = bestPosition.sceneSnapshot.mario.x - currentSearchStartingMarioXPos;
    	System.out.println("bestPosition.sceneSnapshot.mario.x - currentSearchStartingMarioXPos IS: " + temp1);
    	float temp2 = System.currentTimeMillis() - startTime;
    	System.out.println("System.currentTimeMillis() - startTime IS: " + temp2);
    			...
    			//&& (System.currentTimeMillis() - startTime < Math.min(200,timeBudget/2))) //<- this makes the game a bit more jerky, but allows a deeper search in tough situations
    	{
    		...
    		SearchNode current = pickBestPos(posPool);
    		System.out.println("current.sceneSnapshot BEFORE: " + current.sceneSnapshot);
    		currentGood = false;
    		...
    		System.out.println("current.sceneSnapshot.mario.x AFTER: " + current.sceneSnapshot.mario.x);
    		...
    		/*else*/ 
    		if (realRemainingTime - current.remainingTimeEstimated > 0.1)
    		{
    ...
    {    	
    	System.out.println("RUNNING startSearch");
    	...
    	System.out.println("startPos.sceneSnapshot.mario.x: " + startPos.sceneSnapshot.mario.x);
    	...
    	System.out.println("posPool.size() IS: " + posPool.size());
    	/*for (SearchNode node:posPool)
    	{
    		System.out.println("node.sceneSnapshot IS: " + node.sceneSnapshot);
    	}*/
    ...
    private ArrayList<boolean[]> extractPlan()
    {
    	...
    	System.out.println("INSIDE extractPlan()");
    	System.out.println("bestPosition IS: " + bestPosition);
    ...
    private SearchNode pickBestPos(ArrayList<SearchNode> posPool)
    {
    	...
    	{	    		
    		...
    		if (currentCost < bestPosCost)
    		{
    			...
    			System.out.println("LOWEST COST IS: " + currentCost);
    		}
    	}
	...
	public boolean[] optimise()
	{
        ...
        System.out.println("levelScene STATE 1: " + levelScene);
        System.out.println("levelScene.mario.x STATE 1: " + levelScene.mario.x);
        
        //System.out.println("currentState.mario.x IS: " + currentState.mario.x);
        /*if (workScene == null)
        ...
        if (ticksBeforeReplanning <= 0 || currentActionPlan.size() == 0)
        {
        	...
        	System.out.println("currentActionPlan.size() IS: " + currentActionPlan.size());
        	...
        	for (int i = 0; i < planAhead; i++)
        	{
        		...
        		System.out.println("INSIDE planAhead, levelScene.mario.x IS: " + levelScene.mario.x);
        	}
        ...
        System.out.println("RIGHT BEFORE SEARCH!");	
		search(startTime);
    	//workScene = backupState();
